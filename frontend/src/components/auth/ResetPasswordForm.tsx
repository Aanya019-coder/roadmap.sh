...
const ResetPasswordForm: React.FC<{ token: string }> = ({ token }) => {
    ...
    return (
        <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto', background: '#111', padding: '2rem', borderRadius: '16px', border: '1px solid #262626' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: 700, margin: '0 0 1rem', textAlign: 'center' }}>Set New Password</h2>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                    <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, margin: '0 0 0.5rem', color: '#e5e5e5' }}>New Password</label>
                    <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} style={{ width: '100%', padding: '0.75rem 1rem', background: '#1c1c1c', border: `1px solid #262626`, borderRadius: '8px', color: '#fff', outline: 'none' }} placeholder="••••••••" />
                </div>
                ...
                <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.875rem', background: '#16a34a', color: '#fff', fontWeight: 600, border: 'none', borderRadius: '8px', cursor: loading ? 'not-allowed' : 'pointer', marginTop: '0.5rem' }}>
                    {loading ? 'Setting...' : 'Update Password'}
                </button>
            </form>
        </div>
    );
